...
DROP PROCEDURE LTN_FEATURES_BY_USER;
CREATE PROCEDURE LTN_FEATURES_BY_USER(IN task_id INT, IN username VARCHAR(255), IN max_size INT, OUT train_data T_TRAIN_DATA)
	...
	FROM (
		...
		FROM(
			...
			FROM (
				...
				FROM LTN_DEVELOP.ENTITIES E1
				CROSS JOIN LTN_DEVELOP.ENTITIES E2
				LEFT OUTER JOIN LTN_DEVELOP.PAIRS P ON E1.ID = P.E1_ID AND E2.ID = P.E2_ID
				JOIN LTN_DEVELOP.USER_DOCUMENTS UD ON E1.USER_DOC_ID = UD.ID AND E2.USER_DOC_ID = UD.ID
				JOIN LTN_DEVELOP.DOCUMENTS D ON UD.DOCUMENT_ID = D.ID
				JOIN LTN_DEVELOP.OFFSETS O1 ON O1.ENTITY_ID = E1.ID AND O1.USER_DOC_ID=UD.ID
				JOIN LTN_DEVELOP.OFFSETS O2 ON O2.ENTITY_ID = E2.ID AND O2.USER_DOC_ID=UD.ID
				...
				JOIN LTN_DEVELOP.POS_TAGS POSTAGS ON FTI.TA_TYPE = POSTAGS.POS
				WHERE D.TASK = :task_id
				AND UD.USER_ID = :username
			...
			WHERE TOKEN NOT IN (SELECT STOPWORD FROM LTN_DEVELOP.STOPWORDS)
		)
...
DROP PROCEDURE LTN_FEATURES;
CREATE PROCEDURE LTN_FEATURES(IN task_id INT, IN max_size INT, OUT train_data T_TRAIN_DATA)
LANGUAGE SQLSCRIPT AS
BEGIN
DECLARE FTI LTN_DEVELOP.T_INDEX;
...
CALL LTN_FEATURES_BY_USER(1, 'DDI-IMPORTER', -1, ?);
CALL LTN_FEATURES_BY_USER(1, 'DDI-TEST_DATA', -1, ?);

CALL LTN_FEATURES(1, -1, ?);

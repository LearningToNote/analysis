data = read.csv("~/Documents/python_hana/analysis/rData/classes_pos.csv")
...
p_before_dtm <- DocumentTermMatrix(p_before_Corpus, control = list(removeStopwords=FALSE))

p_between_Corpus = Corpus(VectorSource(data$P_BETWEEN))
p_between_dtm <- DocumentTermMatrix(p_between_Corpus, control = list(removeStopwords=FALSE))

p_after_Corpus = Corpus(VectorSource(data$P_AFTER))
p_after_dtm <- DocumentTermMatrix(p_after_Corpus, control = list(removeStopwords=FALSE))
